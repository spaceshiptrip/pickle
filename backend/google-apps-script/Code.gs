...
function doPost(e) {
  ...
  if (action === "auth.requestmagiclink")
    return json_(auth_requestMagicLink_(payload));
...
function auth_updateLogin_(payload, ctx) {
  ...
  if (isLoginHashTaken_(h, ctx.userId))
...
function auth_updatePin_(payload, ctx) {
  ...
  // verify old pin
  if (!verifyPin_(oldPin, ctx.user.PinHash))
    return { ok: false, error: "invalid_old_pin" };
...
function requireAuth_(e, payload) {
  ...
  if (e && e.parameter && e.parameter.sessionId)
    sid = String(e.parameter.sessionId).trim();

  // allow sessionId in JSON body for POSTs
  if (!sid && payload && payload.sessionId)
    sid = String(payload.sessionId).trim();
...
function canSeeReservation_(ctx, resObj) {
  ...
  var userGroup =
    ctx && ctx.user && ctx.user.Group
      ? String(ctx.user.Group).trim().toLowerCase()
      : "";
  if (!userGroup) return false;

  var groups = parseGroupCsv_(resObj && resObj.VisibleToGroups).map(
    function (g) {
...
function readTable_(sheet) {
  ...
  var rows =
    values.length < 2
      ? []
      : values.slice(1).filter(function (r) {
          return r.join("").trim() !== "";
        });
...
function createApprovalRequest_(email, name) {
  ...
  if (!t.header || t.header.length === 0)
    throw new Error("Approvals sheet missing header");
...
function upsertRowById_(sheet, idColName, rowObj) {
  ...
  for (var i = 0; i < t.rows.length; i++) {
    if (String(t.rows[i][idCol]) === String(rowObj[idColName])) {
      foundRowIndex = i;
      break;
    }
  }

  var arr = t.header.map(function (h) {
...
function isAllowlistedForAny_(ctx, items) {
  ...
  return (items || []).some(function (it) {
    ...
    if (userGroup) {
      var groups = parseGroupCsv_(it && it.VisibleToGroups).map(function (g) {
        return String(g).trim().toLowerCase();
      });
...
function listReservations(ctx) {
  ...
  var items = rt.rows.map(function (r) {
    ...
    return {
      ...
      Status: String(r[rIdx["Status"]] || "reserved")
        .toLowerCase()
        .trim(),
      Visibility: hasVisibility
        ? normalizeVisibility_(r[rIdx["Visibility"]])
        : "member",
      VisibleToUserIds: hasAllow
        ? String(r[rIdx["VisibleToUserIds"]] || "").trim()
        : "",
      VisibleToGroups: hasGroupAllow
        ? String(r[rIdx["VisibleToGroups"]] || "").trim()
        : "",
  ...
  items = items.filter(function (it) {
    return canSeeReservation_(ctx, it);
  });
...
function listAttendance_(e, ctx) {
  var resId =
    e && e.parameter && e.parameter.reservationId
      ? String(e.parameter.reservationId)
      : "";
  var month =
    e && e.parameter && e.parameter.month ? String(e.parameter.month) : "";
...
function signup_(payload, ctx) {
  ...
  if (ctx.role === "admin") {
    ...
    var incoming =
      payload.players && payload.players.length
        ? payload.players.map(function (p) {
            return String(p || "").trim();
          })
        : [];
    ...
    var hasMe = incoming.some(function (p) {
      return p && p.toLowerCase() === myName.toLowerCase();
    });
  ...
  if (
    payload &&
    payload.perPersonAmount !== undefined &&
    payload.perPersonAmount !== null &&
    payload.perPersonAmount !== ""
  ) {
  ...
  var rows = players.map(function (p) {
    var rowObj = {};
    attendees.header.forEach(function (h) {
      rowObj[h] = "";
    });
    ...
    return attendees.header.map(function (h) {
      return rowObj[h];
    });
  });
...
function unrsvp_(payload, ctx) {
  ...
  if (idx["Present (1/0)"] === undefined)
    return { ok: false, error: "missing_present_column" };
  ...
  if (tableRowIndex < 0 || tableRowIndex >= t.rows.length)
  ...
  if (!isAdmin) {
    if (idx["UserId"] === undefined)
      return { ok: false, error: "missing_userid_column" };
...
function markPaid_(payload) {
  ...
  if (!payload.reservationId || !payload.player)
    return { ok: false, error: "bad_request" };
...
function upsertReservation_(payload) {
  ...
  var status = String(payload.Status || "reserved")
    .toLowerCase()
    .trim();
  ...
  for (var i = 0; i < rs.rows.length; i++) {
    ...
    if (String(r[idx["Id"]]) === String(id)) {
      return {
        ...
        Status: String(r[idx["Status"]] || "reserved")
          .toLowerCase()
          .trim(),
...
function listFeesFor_(reservationId) {
  ...
  return fs.rows
    .filter(function (r) {
      return String(r[idx["ReservationId"]]) === reservationId;
    })
    .map(function (r) {
      return {
...
function auth_loginWithPin_(payload) {
  ...
  if (!verifyPin_(pin, user.PinHash))
    return { ok: false, error: "invalid_login" };
...
function auth_requestMagicLink_(payload) {
  ...
  if (!email)
    return {
      ok: true,
      message: "If approved, you will receive an email shortly.",
    };
  ...
  if (
    user &&
    user.Active &&
    String(user.Role).toLowerCase().trim() === "guest"
  ) {
    ...
    return {
      ok: true,
      message:
        "Welcome back! You are already approved. A new magic link has been sent to your email.",
    };
  }
  ...
  if (findPendingApprovalByEmail_(email)) {
    return {
      ok: true,
      message:
        "Your request is currently pending approval. We will email you once approved!",
    };
  }
  ...
  return {
    ok: true,
    message:
      "Your access request has been submitted! We will email you a magic link once approved.",
  };
}
...
function reportSummary_(sessionUser, params) {
  ...
  var to = toStr ? parseYmd_(toStr) : today;
  ...
  var totals = { plays: 0, charges: 0, paid: 0, balance: 0 };
  var byRes = {}; // key: ReservationId -> aggregate
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    if (playerName) byRes[reservationId].players.push(playerName);
    byRes[reservationId].charges += charge;
    byRes[reservationId].paid += paid;
  }

  totals.balance = round2_(totals.charges - totals.paid);

  // Build array sorted desc by date
  var byReservation = Object.keys(byRes)
    .map(function (k) {
      var it = byRes[k];
      // de-dupe player list while preserving order
      var seen = {};
      it.players = it.players.filter(function (n) {
        if (seen[n]) return false;
        seen[n] = true;
        return true;
      });
      it.balance = round2_(it.charges - it.paid);
      it.charges = round2_(it.charges);
      it.paid = round2_(it.paid);
      return it;
    })
    .sort(function (a, b) {
  ...
  return {
    ...
    totals: {
      plays: totals.plays,
      charges: round2_(totals.charges),
      paid: round2_(totals.paid),
...
function findUserById_(userId) {
  ...
  for (var i = 0; i < t.rows.length; i++) {
    var r = t.rows[i];
    if (String(r[idx["UserId"]]) === String(userId))
      return rowToUser_(t.header, r, idx);
  }
  return null;
}
...
function findUserByLoginHash_(loginHash) {
  ...
  if (idx["LoginHash"] === undefined)
    throw new Error("Users sheet missing LoginHash column");

  for (var i = 0; i < t.rows.length; i++) {
    var r = t.rows[i];
    var h = String(r[idx["LoginHash"]] || "").trim();
    if (h && h === String(loginHash).trim())
      return rowToUser_(t.header, r, idx);
  }
  return null;
}
...
function rowToUser_(header, row, idx) {
  ...
  return {
    ...
    LoginHash:
      idx["LoginHash"] !== undefined
        ? String(row[idx["LoginHash"]] || "").trim()
        : "",
    Venmo: String(row[idx["Venmo"]] || "").trim(),
    PinHash: String(row[idx["PinHash"]] || "").trim(),
    Active: active,
    Group:
      idx["Group"] !== undefined ? String(row[idx["Group"]] || "").trim() : "",
  };
}
...
function getPinSalt_() {
  ...
  if (!salt)
    throw new Error(
      "PIN salt not set. Run setPinSalt_() once from the script editor.",
    );
  return salt;
}
...
function createAuthToken_(userId, ttlMin) {
  ...
  var t = readTable_(sh);
  if (!t.header || t.header.length === 0)
    throw new Error("AuthTokens sheet missing header");
...
function createSession_(userId, role) {
  ...
  var t = readTable_(sh);
  if (!t.header || t.header.length === 0)
    throw new Error("Sessions sheet missing header");
...
function normalizeVisibility_(v) {
  v = String(v || "member")
    .toLowerCase()
    .trim();
  if (v === "admin" || v === "memberplus" || v === "member" || v === "guest")
    return v;
  return "member";
}
...
function generateMyHash() {
  ...
  const salt =
    PropertiesService.getScriptProperties().getProperty("PIN_SALT_V1");
...
function migrateUsersToLoginHash_() {
  ...
  if (idx["LoginHash"] === undefined)
    throw new Error("Add LoginHash column first");
  ...
  for (var i = 0; i < t.rows.length; i++) {
    ...
    var email =
      idx["Email"] !== undefined ? normalizeEmail_(r[idx["Email"]]) : "";
    var phone =
      idx["Phone"] !== undefined ? normalizePhone_(r[idx["Phone"]]) : "";
